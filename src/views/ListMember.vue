        ...
        <el-table :data="tableData"
          ...
          <el-table-column label="操作"
            ...
            <template slot-scope="scope">
              <el-button @click="updateMember(scope.row)"
                         type="primary"
                         size="small">
                <i class="el-icon-edit"></i>
                <span>编辑</span>
              </el-button>
              <el-button @click="deleteMember(scope.row)"
                         type="danger"
                         size="small">
...
export default {
  methods: {
    // 编辑
    updateMember(row) {
      this.$router.push({ name: 'ResetPass', params: row })
    },
    // 删除
    deleteMember(row) {
      Http.member_del(row._id)
        .then(rst => {
          console.log(rst)
          this.getMemberCount()
    ...
    // 获取权限列表
    getMemberList(currpage) {
      Http.member_list(currpage)
        .then(rst => {
    ...
    getMemberCount() {
      Http.member_count()
        .then(rst => {
          ...
          this.getMemberList(this.currpage)
        })
        .catch(err => {
          console.log(err)
        })
